...
Orders require Ed25519 signatures over a 169-byte message hash.

## Using the SDK (Recommended)

The SDK handles the 169-byte message hash construction and signing:
...
Orders are signed over a 169-byte Keccak256 hash with this layout:
...
The `salt` is a unique value per order (e.g. nanosecond timestamp) ensuring distinct hashes even when all other fields match. The signature is Ed25519 over `keccak256(message_bytes)`.
...
function createOrderHash(order: {
  ...
  // salt (u64 LE)
  view.setBigUint64(8, order.salt, true);
  // maker (32 bytes)
  bytes.set(order.maker, 16);
  // market (32 bytes)
  bytes.set(order.market, 48);
  // base_token (32 bytes)
  bytes.set(order.base_token, 80);
  // quote_token (32 bytes)
  bytes.set(order.quote_token, 112);
  // side (1 byte)
  view.setUint8(144, order.side);
  // amount_in (u64 LE)
  view.setBigUint64(145, order.amount_in, true);
  // amount_out (u64 LE)
  view.setBigUint64(153, order.amount_out, true);
  // expiration (i64 LE)
  view.setBigInt64(161, order.expiration, true);
...
def create_order_hash(
    ...
    # Pack 169 bytes
    message = struct.pack('<Q', nonce)   # u64 LE
    message += struct.pack('<Q', salt)   # u64 LE
...
| `salt` | integer | Unique per-order value (e.g. nanosecond timestamp) |
