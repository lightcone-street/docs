...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut order = FullOrder::new_bid(BidOrderParams {
        ...
        maker_amount: 650_000,   // amount_in: quote tokens you give (denom in token lamports)
        taker_amount: 1_000_000, // amount_out: base tokens you receive (denom in token lamports)
        expiration: 0,
    });
...
const order = createSignedBidOrder(
  {
    ...
    makerAmount: 650_000n,   // amount_in: quote tokens you give (token lamports)
    takerAmount: 1_000_000n, // amount_out: base tokens you receive (token lamports)
    expiration: 0n,
  },
  keypair
);
...
order = create_signed_bid_order(
    BidOrderParams(
        ...
        maker_amount=650_000,    # amount_in: quote tokens you give (token lamports)
        taker_amount=1_000_000,  # amount_out: base tokens you receive (token lamports)
        expiration=0,
    ),
    keypair,
)
...
Orders are signed with Ed25519 over a Keccak256 hash of a 169-byte payload, structured as follows.
...
The `salt` field is a unique value per order (e.g. nanosecond timestamp) that ensures distinct order hashes even when all other fields are identical.

<Note>
`amount_in` and `amount_out` must be in token lamports (smallest token unit). API responses return scaled decimal strings for display, but order submissions use lamport values specific to the token being referenced.
</Note>
...
const order = {
  ...
  nonce: Date.now(),
  salt: Number(process.hrtime.bigint() % BigInt(2**53)),
  market_pubkey: "FjVJXzKknRoxgdQHNfdMkzq2vswGacM3MHiZhahczBsL",
  base_token: "5BNoPgYemyrbS4ub7MTuKczoYA7qtJZTj5m7xVanupPR",
  quote_token: "CrhujZLfmzffVjwUTk6wL1mYeF9nRkx4MBvHmEotnaDF",
  side: 0,
...
To cancel all orders, sign the string `"cancel_all:<user_pubkey>:<orderbook_id>:<timestamp>:<salt>"` where `timestamp` is the current Unix time in seconds, `orderbook_id` is the optional scope (empty string if cancelling all), and `salt` is a random unique string for one-time use.
...
let timestamp = SystemTime::now()
    ...
    .as_secs() as i64;
let orderbook_id = "5BNoPgYe_CrhujZLf"; // or "" to cancel all orderbooks
let salt = uuid::Uuid::new_v4().to_string();
let message = format!("cancel_all:{}:{}:{}:{}", keypair.pubkey(), orderbook_id, timestamp, salt);
...
const orderbookId = "5BNoPgYe_CrhujZLf"; // or "" to cancel all orderbooks
const salt = randomUUID();
const message = `cancel_all:${keypair.publicKey.toBase58()}:${orderbookId}:${timestamp}:${salt}`;
...
orderbook_id = "5BNoPgYe_CrhujZLf"  # or "" to cancel all orderbooks
salt = str(uuid.uuid4())
message = f"cancel_all:{pubkey}:{orderbook_id}:{timestamp}:{salt}"
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 1. Fetch a single-use nonce from the server
    let nonce_resp: serde_json::Value = client
        .get("https://tapi.lightcone.xyz/api/auth/nonce")
        .send()
        .await?
        .json()
        .await?;
    let nonce = nonce_resp["nonce"].as_str().unwrap();

    // 2. Build the sign-in message with the nonce
    ...
    // 5. Extract JWT from Set-Cookie header
    let token = response
        .headers()
        .get(SET_COOKIE)
...
// 1. Fetch a single-use nonce from the server
const nonceResp = await fetch('https://tapi.lightcone.xyz/api/auth/nonce');
...
# 1. Fetch a single-use nonce from the server
nonce_resp = requests.get('https://tapi.lightcone.xyz/api/auth/nonce')
