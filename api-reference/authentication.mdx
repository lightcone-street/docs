...
1. **Construct the payload** with all required fields
...
let cancel_all_result = client.cancel_all_orders(
    "FjVJXzKknRoxgdQHNfdMkzq2vswGacM3MHiZhahczBsL",
    &keypair,
).await?;
```
...
const cancelAllResult = await client.cancelAllOrders({
  marketPubkey: "FjVJXzKknRoxgdQHNfdMkzq2vswGacM3MHiZhahczBsL",
}, keypair);
```
...
To cancel all orders, sign the string `"cancel_all:<user_pubkey>:<timestamp>"` where `timestamp` is the current Unix time in seconds. The timestamp must be within 30 seconds of present time.
