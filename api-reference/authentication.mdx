...
JWT authentication **(TO CHANGE:)** consists of the user’s API credentials (apiKey, secret, passphrase) generated from a wallet's signature. These are used for specific API methods, as well as websocketsc. Requests are signed using [cryptographic method].
...
For convenience, API and websocket responses read and return decimal strings _(e.g. `"0.65"`, `"1.5"`)_ automatically scaled from the onchain lamport units in all its response. 

However, order submission requires **lamports**-denominated amounts, since the payload is signed and executed onchain.
...
- wBTC has 8 decimals, so all conditional BTC tokens like Invasion-BTC inherit 8 decimals.


When querying for a list of markets, decimals are listed under each deposit token(?)
...
<Accordion title="(Code Example) Sign payloads without the SDK">
<CodeGroup>
