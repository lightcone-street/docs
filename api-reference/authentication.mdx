...
When submitting orders, you must sign the order data with a Solana account:

1. **Create the order payload** with all required fields
...
<Note>
`maker_amount` and `taker_amount` must be in lamports (smallest token unit). API responses return scaled decimal strings for display, but order submission uses lamport values.
</Note>
