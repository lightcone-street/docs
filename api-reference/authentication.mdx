...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = client
        .post("https://api3.internalcone.com/api/orders/submit")
        .json(&order_with_sig)
        .send()
        .await?;
...
// Submit order
const response = await fetch('https://api3.internalcone.com/api/orders/submit', {
  method: 'POST',
...
response = requests.post(
    "https://api3.internalcone.com/api/orders/submit",
    json=order
)
...
| `maker_amount` | integer | Raw integer amount (unscaled) |
| `taker_amount` | integer | Raw integer amount (unscaled) |
...
<Note>
`maker_amount` and `taker_amount` must be raw integers (not scaled decimals) because they are part of the signed hash. API responses return scaled decimal strings for display, but order submission uses raw values.
</Note>
