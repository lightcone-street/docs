...
flowchart TD
  ...
  %% --- Two parallel orderbooks ("universes") ---
  subgraph R["Universe: Republicans win (RWin orderbook)"]
    direction TB
    RPAIR[(RWin-BTC / RWin-USDC)]
    RU --> RPAIR
    RB --> RPAIR
    RPAIR --> RTR[Trade / set impact exposure in RWin]
  end

  subgraph DWIN["Universe: Democrats win (DWin orderbook)"]
    direction TB
    DPAIR[(DWin-BTC / DWin-USDC)]
    DU --> DPAIR
    DB --> DPAIR
    DPAIR --> DTR[Trade / set impact exposure in DWin]
  end

  %% --- "Picking a universe" behavior ---
  U --> P{Pick which universe to trade in}
  P -->|Trade RWin tokens| RTR
  P -->|Trade DWin tokens| DTR
  P -->|Do nothing in one universe| HOLD[Leave the other universe tokens untouched]

  %% --- Resolution ---
  E[Event resolves: election outcome known] --> W{Which universe wins?}

  %% If Republicans win
  W -->|Republicans win| RREDEEM[RWin tokens redeem 1:1 to real BTC/USDC]
  W -->|Republicans win| DZERO[DWin tokens become worthless]
  RREDEEM --> SPOTR[(Spot BTC / Spot USDC)]

  %% If Democrats win
  W -->|Democrats win| DREDEEM[DWin tokens redeem 1:1 to real BTC/USDC]
  W -->|Democrats win| RZERO[RWin tokens become worthless]
  DREDEEM --> SPOTD[(Spot BTC / Spot USDC)]
```
