...
<ResponseField name="bids[].price" type="string">
  Price level as decimal string (e.g., `"0.650000"`)
</ResponseField>

<ResponseField name="bids[].size" type="string">
  Total size at this price level as decimal string. `"0.000000"` means delete the price level.
</ResponseField>
...
class OrderbookManager {
  constructor(reconnectCallback) {
    this.bids = new Map(); // price string -> size string
    this.asks = new Map();
    this.lastSeq = 0;
  ...
  getBestBid() {
    const prices = [...this.bids.keys()].map(parseFloat);
    return Math.max(...prices).toFixed(6);
  }

  getBestAsk() {
    const prices = [...this.asks.keys()].map(parseFloat);
    return Math.min(...prices).toFixed(6);
  }
}
