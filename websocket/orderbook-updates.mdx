...
description: 'Real-time orderbook data with efficient delta updates'
---

## Overview

The `book_update` channel provides efficient orderbook updates using delta mode:
1. **Initial snapshot** with full orderbook state
2. **Delta updates** every 50ms with only changed price levels

<Info>
Delta mode uses ~95% less bandwidth than full snapshots while providing faster updates (50ms vs 200ms).
</Info>
